...
/*
Воркеры (Workers) и отправитель данных (Publisher) спроектированы в отдельных горутинах.
По сигналу закрытия приложения происходит изменение глобальной переменной в atomic.Bool, далее следует инициализация
закрытия воркеров через передачу из отправителя состояния закрытия каналов и закрытие самой горутины отправителя.
*/
